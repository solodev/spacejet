<footer role="contentinfo" class="py-3 py-md-5">
	<div class="container">
		<div class="row d-flex align-items-center justify-content-between">
          <div class="col-md-2">
              <a href="/" class="d-block p-3 p-sm-0"><img alt="" aria-role="logo" class="img-fluid" src="/_/images/space-jet-logo.png" /> </a>
          </div>
          <div class="col-md-8 my-3 my-md-0">
              <ul  class="list-unstyled list-inline mb-0 d-flex flex-wrap justify-content-center footer-nav">
                  <li><a href="tel:4078981961" class="text-black pr-3">(407) 898-1961</a></li>
                  <li><a href="mailto:info@spacejet.com" class="text-black px-3">info@spacejet.com</a></li>
                  <li><a href="/about.stml" class="text-black px-3">About</a></li>
                  <li><a href="/contact-us.stml" class="text-black pl-3">Contact</a></li>
              </ul>
          </div>
          <div class="col-md-2 d-none d-md-block pl-xl-6">
              <ul class="nav flex-row justify-content-between flex-wrap align-items-center social">
              <li class="nav-item mb-0"><a class="d-block py-4" href="https://www.facebook.com/Solodev/"><i class="fab fa-facebook-f fa-lg"  aria-hidden="true"></i></a></li>
              <li class="nav-item mb-0"><a class="d-block py-4" href="https://www.instagram.com/digitalusagency/?hl=en"><i class="fab fa-instagram fa-lg"></i></a></li>
              <li class="nav-item mb-0"><a class="d-block py-4" href="https://www.linkedin.com/company/solodev"><i class="fab fa-linkedin-in fa-lg"></i></a></li>
            </ul>
          </div>
	</div>
	<div class="row d-flex align-items-center mt-3">
		<div class="col-md-8 text-center text-md-left">
			<ul class="list-unstyled list-inline smaller">
				<li>&copy;2020 SpaceJet. All rights reserved worldwide and off planet.</li>
				<li><a href="#" class="text-black px-3">Terms Of Use</a></li>
				<li><a href="#" class="text-black px-3">Privacy Policy</a></li>
			</ul>
		</div>
		<div class="col-md-4 d-flex justify-content-center justify-content-md-end">
			<div class="font-size d-flex justify-content-end">
				<div class="change-font d-flex align-items-center justify-content-between">
					<p><small>Text size</small>
					<a class="size font-size-1 active px-2 border border-black rounded ml-3" href="#" arial-label="click to make font size smaller">A</a>
					<a class="size font-size-2 px-2 border border-black rounded" href="#" arial-label="click to make font size large">A</a>
					<a class="size font-size-3 px-2 border border-black rounded" href="#" arial-label="click to make font size extra large">A</a>
                    </p>
				</div>
			</div>
		</div>
	</div>
</div>
<footer>